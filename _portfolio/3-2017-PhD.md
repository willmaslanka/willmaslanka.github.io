...
The ASMEx snow slab data were used with a flux coefficient model to calculate six flux absorption and scattering coefficients. The six flux scattering coefficients in the vertical polarization were used with a theoretical absorption coefficient model to create a new semi-empirical extinction coefficient ([k_e]) eliminating the need to use subjective observations.

$$k_e = (4\pi(F x 10^{9}))(sqrt(mu_{0} ep_0 e'snow)(sqrt(0.5(sqrt(1+(ep''snow/ep'snow)^{2}))-1)) + 0.0065(D_{o})^{2.12}(F)^{2.12}$$

where F is frequency (GHz), mu_o is the permeability of free space (4pi x 10^-7 Hm-1), ep_0 is the permittivity of free space (8.85x10^-12 F m-1), ep'snow and ep''snow are the real and imaginary parts of the permittivity of dry snow respectively, and D_o is the optical diameter (mm, inversely proportional to the Specific Surface Area).])
